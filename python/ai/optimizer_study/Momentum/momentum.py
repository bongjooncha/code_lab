...
# Momentom 구현
theta_mom = np.random.randn(2, 1)  # 세타값을 무작위로 생성
theta_mom_path = []                # 앞으로 생성될 세타 쌍을 저장하는 공간
...
def animate(i):
    ...
    plt.plot(X, X_b.dot(theta_mom_path[i]), 'r-')  # 선형 회귀 선 플롯
    theta0 = float(theta_mom_path[i, 0])
    theta1 = float(theta_mom_path[i, 1])
...
# 애니메이션 설정
fig, ax = plt.subplots()
ani = FuncAnimation(fig, animate, frames=len(theta_mom_path), interval=50)
